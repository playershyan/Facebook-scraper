"""
Quick test script - scrapes only 5 pages for testing.
"""
import asyncio
import os
import sys
...
TEST_PAGES = 5
OUTPUT_CSV = "test_output_5_pages.csv"
...
async def test_scrape_5_pages():
    """Test scraping 5 pages."""
    print("=" * 60)
    print("TEST: Scraping 5 Pages")
    print("=" * 60)
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        for page_num in range(1, TEST_PAGES + 1):
            try:
                ...
                for idx, (listing_url, preview_title) in enumerate(listing_data_tuples[:5], 1):  # Limit to 5 per page for test
                    if is_duplicate_listing(listing_url, seen_urls):
                        stats['duplicates_skipped'] += 1
                        continue

                    print(f"    [{idx}/5] {preview_title[:50]}...")
